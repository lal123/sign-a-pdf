...
if($pdf_id != '') {
    if($is_signed_in) {
        ...
        $doc_signed = ($_SESSION['docs'][$pdf_id]['signed'] == 1);
    }
...
?>
            <div class="btn-group mx-auto mb-2" role="group" aria-label="">
                <a href="javascript:void(0)" id="signButton" onclick="<?php if((php_uname("n") == 'alain-520-1080fr') || (isset($user) && ($user['user_id'] == 1))) { ?>return docs.initSign('<?php echo $pdf_id; ?>'); <?php } ?>return false;" class="btn btn-primary btn-lg dark-cyan<?php if($doc_signed == 1) { echo ' disabled'; } ?>"><?php echo $tr['DOCS.SIGN_THIS_DOC']; ?></a>
            </div>
            <div class="btn-group mx-auto mb-2" role="group" aria-label="">
