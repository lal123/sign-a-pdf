...
if(file_exists($img_dir . '/' . $pdf_id .'.png')) {
    ...
    while(file_exists($img_dir . '/' . $pdf_id . '-' . $img_numb . '.png')) {
        ...
        echo '<div class="col col-lg-' . $bs_dir . ' col-md-' . $bs_dir . ' col-sm-' . $bs_dir . ' col-xs-' . $bs_dir . '"><img style="max-width: 500px;" src="/' . UPLOAD_DIR . '/img/' . $pdf_id . '-' . $img_numb . '.png' . '" alt="" border= "0" /></div>';
        $col++;
        if($col > $nb_cols) {
            echo '</div>';
            $col = 1;
